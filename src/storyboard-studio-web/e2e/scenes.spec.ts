...
test('local lighting controls persist and draw after reopening', async ({ page }) => {
  ...
  await page.getByText('Color and local lights', { exact: true }).click()
  await page.getByLabel('Filmic light response').check()
  await page.getByLabel('Show floor grid').uncheck()
  await page.getByRole('button', { name: 'Add local light', exact: true }).click()
  await page.getByLabel('Local light 1 brightness').fill('75')
  await page.getByLabel('Local light 1 X', { exact: true }).fill('2.5')
  await page.getByLabel('Cast shadows', { exact: true }).check()
  await page.getByTestId('scene-save').click()
  await expect(page.getByTestId('scene-version')).toContainText('Version 2')
  await page.reload()
  await page.getByRole('button', { name: 'Scene', exact: true }).click()
  await page.getByText('Color and local lights', { exact: true }).click()
  await expect(page.getByLabel('Local light 1 brightness')).toHaveValue('75')
  await expect(page.getByLabel('Local light 1 X', { exact: true })).toHaveValue('2.5')
  await expect(page.getByLabel('Filmic light response')).toBeChecked()
  await expect(page.getByLabel('Show floor grid')).not.toBeChecked()
