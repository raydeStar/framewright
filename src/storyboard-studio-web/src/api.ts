...
function directorQuery(view: DirectorShotView) {
...
export const studioApi = {
  ...
  webMcpAssetContext: (view: DirectorAssetView, signal?: AbortSignal) => request<WebMcpEnvelope<Record<string, unknown>>>(`/api/webmcp/director/assets/${view.assetId}/context?contentHash=${encodeURIComponent(view.contentHash)}&directorMode=${view.directorMode}`, { signal }),
  webMcpAssetObservation: (view: DirectorAssetView, stateToken: string, signal?: AbortSignal) => request<WebMcpEnvelope<Record<string, unknown>>>(`/api/webmcp/director/assets/${view.assetId}/observation?contentHash=${encodeURIComponent(view.contentHash)}&stateToken=${encodeURIComponent(stateToken)}`, { signal }),
  ...
  generationSetup: () => request<GenerationSetup>('/api/setup/generation'),
  saveGenerationSetup: (body: GenerationSetupChange) => request<GenerationSetup>('/api/setup/generation', { method: 'PUT', headers: { 'X-Storyboard-Studio': '1' }, body: JSON.stringify(body) }),
  testComfyUi: (endpoint: string) => request<ComfyUiConnectionTest>('/api/setup/comfyui/test', { method: 'POST', headers: { 'X-Storyboard-Studio': '1' }, body: JSON.stringify({ endpoint }) }),
  ...
  generateModel: (sourceAssetId: string, name: string, size: string, glassColour?: string, detail?: string, headEnd?: string) =>
  ...
  resolveSceneAnnotation: (annotationId: string) => request<SceneAnnotationSummary>(`/api/scene-annotations/${annotationId}/resolve`, { method: 'POST' }),
  sceneProposals: (sceneId: string) => request<SceneProposalSummary[]>(`/api/scenes/${sceneId}/proposals`),
  ...
  rejectSceneBlockout: (planId: string) => request<SceneBlockoutPlanSummary>(`/api/scene-blockouts/${planId}/reject`, { method: 'POST' }),
  applySceneBlockout: (planId: string, sceneName?: string) =>
    request<SceneSummary>(`/api/scene-blockouts/${planId}/apply`, { method: 'POST', body: JSON.stringify({ sceneName: sceneName ?? null }) }),
  webMcpProposeSceneBlockout: (body: Record<string, unknown>, signal?: AbortSignal) =>
    request<WebMcpEnvelope<SceneBlockoutPlanSummary>>('/api/webmcp/scene-blockouts', { method: 'POST', body: JSON.stringify(body), signal }),
  webMcpProposeSceneEdit: (body: Record<string, unknown>, signal?: AbortSignal) =>
    request<WebMcpEnvelope<SceneProposalSummary>>('/api/webmcp/scene-proposals', { method: 'POST', body: JSON.stringify(body), signal }),
  ...
  assetUsage: () => request<AssetUsageSummary[]>('/api/assets/usage'),
  assetPosters: () => request<string[]>('/api/assets/posters'),
