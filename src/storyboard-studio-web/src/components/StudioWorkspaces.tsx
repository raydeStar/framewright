...
export function BoardWorkspace({ studio, selectedId, initialView, banner, onViewChange, onSelect, onOpen, onCreateShot, onCreateAuthority, onEditAuthority, onOpenLibrary, onChanged }: { studio: StudioSnapshot; selectedId: string; banner?: React.ReactNode; initialView?: 'shots' | 'authorities'; onViewChange?: (view: 'shots' | 'authorities') => void; onSelect: (id: string) => void; onOpen: (id: string) => void; onCreateShot: (image?: File) => void; onCreateAuthority: () => void; onEditAuthority: (reference: ReferenceSummary) => void; onOpenLibrary: () => void; onChanged: (message?: string) => void }) {
  ...
  return <main className={`workspace board-workspace ${fileDragging ? 'board-file-drop-active' : ''}`} data-testid="board-workspace"
    ...
    <section className="board-heading">
      ...
      <div className="board-heading-tools">
        ...
        <div className="sequence-metrics" aria-label="Sequence summary">
          ...
          <Metric value={`${ratified}/${studio.shots.length}`} label="Approved" />
    ...
    : view === 'authorities' && studio.references.length === 0 ? <EmptyBoard
      ...
      body="References are the immutable truth for identity, wardrobe, location, props, and style. Shots cite exact versions of them."
      note="Create one with Add reference, or use Import reference to bring in a proven reference from the global library." />
    ...
    : <section id="authority-board-panel" className="authority-wall" role="tabpanel" aria-labelledby="authorities-board-tab" data-testid="reference-board">
      {studio.references.map(reference => <button type="button" className={`authority-card category-${reference.category.toLowerCase()}`} key={reference.id} style={{ '--authority-accent': reference.accent } as React.CSSProperties} onClick={() => onEditAuthority(reference)} aria-label={`Open ${reference.name}, ${reference.category}, approved version ${reference.version}`}>
        <div className="authority-image">{reference.imageUrl ? <img src={reference.imageUrl} alt={`${reference.name} approved reference`} /> : <Artwork variant={reference.visualVariant} label={`${reference.name} approved reference`} />}<span>{reference.category}</span><i><LockKeyhole size={13} />v{reference.version}</i></div>
      ...
      <button type="button" className="new-shot-card new-authority-card" onClick={onCreateAuthority}><Plus size={26} /><span>Add reference</span><small>Approve immutable visual truth</small></button>
    </section>}
  </main>
}
...
const BOARD_FILTERS: { id: BoardFilter; label: string; hint: string; match: (shot: ShotSummary) => boolean }[] = [
  { id: 'all', label: 'All', hint: 'Every shot in the sequence', match: () => true },
  { id: 'working', label: 'In progress', hint: 'Not yet approved', match: shot => shot.approval !== 'Ratified' },
  ...
  { id: 'ratified', label: 'Approved', hint: 'Approved version', match: shot => shot.approval === 'Ratified' },
]
        ...
        {visible.map((shot, index) => {
          ...
          return <article
            ...
            {menuFor === shot.id && <div className="shot-menu" role="menu" onClick={event => event.stopPropagation()}>
              ...
              <button role="menuitem" className="danger" onClick={() => { setMenuFor(undefined); setConfirmDelete(shot) }}><Trash2 size={14} />Delete shot</button>
            </div>}
          </article>
        })}
        <button type="button" className="new-shot-card" onClick={() => onCreateShot()}><Plus size={26} /><span>Add shot</span><small>Describe the shot, then generate or sketch</small></button>
      </div>}
  ...
  return <Dialog className="modal-backdrop" onClose={onCancel} labelledBy="delete-shot-title" initialFocus="[data-dialog-focus]">
    <section className="edit-dialog delete-shot" data-dialog-focus tabIndex={-1}>
      ...
      <div className="edit-dialog-body">
        <p>This removes the shot <strong>{shot.title}</strong> and everything scoped to it — every version, its composition sketch, pinned notes and job history.</p>
        ...
        {ratified && <label className="endpoint-confirm">
          ...
          This shot carries approval evidence. I understand deleting it destroys that record.
        </label>}
      </div>
      <footer>
        <span /><span />
...
export function ShotWorkspace({ studio, shot, comments, references, tool, setTool, directorMode, onDirectorMode, onOpenSetup, setupRevision, onDisplayedRevision, onAddComment, onMoveComment, onResolveReferencePin, onRatify, onOpenReview, onOpenAssets, onEdit, onShotSaved, onShotContractSaved, onCandidatesChanged, onJobQueued, onAskCodex, codexBusy, generationBusy, activeJob, implementationRequest, onImplementationConsumed, sketchLaunch, onSketchLaunchConsumed }: {
  ...
  // Deleting a draft removes it from the version history, and both the button
  // and the Delete key used to do it on the spot. Ask first.
  const [draftToDelete, setDraftToDelete] = useState<CandidateVersionSummary>()
  const requestDeleteCandidate = (candidateId: string) => setDraftToDelete(candidates.find(item => item.id === candidateId))
  ...
  return <main className={`workspace shot-workspace${directorMode ? ' director-mode' : ''}`} data-testid="shot-workspace" style={{
    ...
    <section className="shot-center">
      ...
      <CandidateReviewBar
        ...
        onRedirect={redirect} onDelete={requestDeleteCandidate} onPromote={promoteCandidate} onCompare={candidateId => onOpenReview(candidateId)} onComment={() => { setPinningReference(undefined); setTool('comment') }}
    ...
    <aside className="shot-inspector">
      ...
      <div id="shot-details-panel" className="inspector-scroll" role="tabpanel" aria-labelledby={`${tab}-tab`} tabIndex={0}>
        {tab === 'intent' && <>
          ...
          {shot.approval === 'Ratified' && intentDirty && <p className="intent-authority-warning"><LockKeyhole size={14} />Saving preserves the approved reference and opens a new working sketch version.</p>}
        ...
        {tab === 'refs' && <>
          ...
          {showReferencePicker && !archivedPreview && <div className="reference-picker"><strong>Available references</strong>{studio.references.filter(reference => !shot.referenceIds.includes(reference.id)).map(reference => <button type="button" key={reference.id} disabled={contractBusy} onClick={() => void saveContract([...shot.referenceIds, reference.id], shot.constraints, `${reference.name} added to ${shot.code}.`)}><span><b>{reference.name}</b><small>{reference.category} · v{reference.version}</small></span><Plus size={15} /></button>)}{studio.references.every(reference => shot.referenceIds.includes(reference.id)) && <small>Every approved reference is already attached.</small>}</div>}
          ...
          {references.length === 0 && <p className="inspector-empty">No references attached. Add an approved reference to guide this shot.</p>}
    ...
    {draftToDelete && <ConfirmDialog title={`Delete draft v${draftToDelete.version}?`} confirmLabel={`Delete v${draftToDelete.version}`} busy={candidateBusy} onCancel={() => setDraftToDelete(undefined)} onConfirm={() => void deleteCandidate(draftToDelete.id)}>
      <p>This removes draft <strong>v{draftToDelete.version}</strong> and its notes from this shot's version history. The current version is not affected.</p>
    </ConfirmDialog>}
...
function CandidateReviewBar({ shot, candidates, selectedId, onSelect, onPeek, onRedirect, onDelete, onPromote, onCompare, onComment, busy, generationBusy, error, onDismissError }: {
  ...
  return <div className="review-bar" data-testid="candidate-review-bar">
    <div className="review-strip-row">
      <div className="review-strip" ref={strip} role={ordered.length > 0 ? 'listbox' : 'status'} aria-label="Versions" aria-orientation={ordered.length > 0 ? 'horizontal' : undefined}>
        ...
        {ordered.length === 0 && <span className="review-strip-empty">No versions yet — generate one to start reviewing.</span>}
      </div>
...
function ReferenceRow({ reference }: { reference: ReferenceSummary }) { return <div className="reference-row"><div className="reference-swatch" style={{ '--accent': reference.accent } as React.CSSProperties}>{reference.imageUrl ? <img src={reference.imageUrl} alt="" /> : <Artwork variant={reference.visualVariant} muted />}</div><span><strong>{reference.name}</strong><small>{reference.category} · v{reference.version} · {reference.imageAssetId ? 'visual reference' : 'text only'}</small></span>{reference.imageAssetId ? <BadgeCheck size={16} /> : <CircleAlert size={16} />}</div> }
...
function ShotReferenceCard({ reference, pins, busy, readOnly, onPin, onDropPin, onDragState, onRemove, onResolvePin }: { reference: ReferenceSummary; pins: CommentSummary[]; busy: boolean; readOnly: boolean; onPin: () => void; onDropPin: (x: number, y: number) => void; onDragState: (active: boolean) => void; onRemove: () => void; onResolvePin: (id: string) => void }) {
  ...
  if (!canPlace) return <div className="reference-edit-card reference-nonspatial"><ReferenceRow reference={reference} /><p>{reference.description}</p><small className="reference-lock"><LockKeyhole size={12} />{reference.lockedConstraint}</small><p className="reference-availability-note"><CircleAlert size={14} />{reference.imageAssetId ? `${reference.category} references apply globally and are never placed as image layers.` : 'Text reference only. Add an approved image on the reference card before using spatial placement.'}</p>{pins.length > 0 && <div className="reference-card-pins"><strong>{pins.length} old placement {pins.length === 1 ? 'note' : 'notes'} now treated as semantic context</strong>{pins.map(pin => <div key={pin.id}><MessageCircle size={13} /><span><p>{pin.body}</p><small>v{pin.referenceVersion ?? reference.version}</small></span><button type="button" aria-label={`Remove ${reference.name} placement note`} title="Resolve this obsolete placement" disabled={busy || readOnly} onClick={() => onResolvePin(pin.id)}><X size={13} /></button></div>)}</div>}<div className="reference-actions"><button type="button" className="secondary compact" disabled title={reference.imageAssetId ? 'This reference applies to the whole frame' : 'Attach an approved image to enable placement'}><LockKeyhole size={14} />{reference.imageAssetId ? 'Global reference' : 'No visual to place'}</button><button type="button" className="danger compact" aria-label={`Remove ${reference.name} from shot`} disabled={busy || readOnly} onClick={onRemove}><Trash2 size={14} />Remove</button></div></div>
  ...
  return <Dialog className="modal-backdrop route-backdrop" onClose={onClose} labelledBy="feedback-route-title" initialFocus="textarea">
    <section className="route-modal feedback-route-modal">
      ...
      <div className="route-modal-body">
        ...
        {comments.length > 0 && <div className="feedback-packet-notes">{comments.map((comment, index) => { const reference = references.find(item => item.id === comment.referenceId); return <p key={comment.id} className={reference ? 'reference-feedback' : ''}><strong>{reference ? `${reference.name} · reference v${comment.referenceVersion ?? reference.version}` : `Pin ${index + 1}`}</strong>{comment.body}<small>{Math.round(comment.x * 100)}% across · {Math.round(comment.y * 100)}% down{reference ? ' · exact reference image included' : ''}</small></p> })}</div>}
  ...
  return <Dialog className="modal-backdrop route-backdrop" onClose={onClose} labelledBy="visual-reconcile-title" initialFocus="[data-reconcile-choice]">
    <section className="route-modal visual-reconcile-modal">
      ...
      <div className="route-modal-body">
        ...
        <p className="visual-lock-note"><LockKeyhole size={15} />Each difference starts unresolved, so opening this screen never commits a change. Character, world, style, and reusable reference records stay locked.</p>
...
export function SequenceWorkspace({ studio, selectedId, onSelect, onReordered, onJobQueued }: { studio: StudioSnapshot; selectedId: string; onSelect: (id: string) => void; onReordered: () => void; onJobQueued: (job: JobSummary) => void }) {
  ...
  return <main className="workspace sequence-workspace" data-testid="sequence-workspace">
    ...
    <section className="viewer-row"><div className="sequence-viewer">{currentShot && <ShotArtwork shot={currentShot} />}<div className="viewer-timecode">{timecode}</div></div><div className="sequence-notes"><h3>{studio.project.sequenceCode} · {studio.project.sequenceName}</h3><p>{studio.shots.length} shot{studio.shots.length === 1 ? '' : 's'}, played in board order. Reorder them here or on the board.</p><dl><div><dt>Runtime</dt><dd>{formatTime(totalFrames, studio.project.framesPerSecond)}</dd></div><div><dt>Format</dt><dd>{studio.project.aspectRatio} · {studio.project.framesPerSecond}fps</dd></div><div><dt>Audio</dt><dd>{clips.length} editable clips · {mastering?.clipsWithMedia ?? 0} with media</dd></div></dl></div></section>
...
export function StageBadge({ shot }: { shot: ShotSummary }) {
  const label = shot.approval === 'Ratified' ? `${shot.stage} approved` : shot.approval === 'NeedsWork' ? 'Needs work' : `${shot.stage} v${shot.version}`
