...
export default function AssetWorkspace({ studio, initialAssetId, onEditAuthority, onOpenGeneration, onOpenSequence, onJobQueued, onToast, ...director }: AssetDirectorControls & {
  ...
  const smart = [
    ...
    { id: 'authorities' as const, label: 'References', icon: <BadgeCheck />, count: studio.references.length },
  ...
  const rawGenerationReferences = useMemo<AssetGenerationReference[]>(() => [
    ...
    ...studio.references.filter(reference => reference.imageAssetId && reference.imageUrl).map(reference => ({ id: `authority:${reference.id}`, assetId: reference.imageAssetId!, label: reference.name, detail: `${reference.category} · reference v${reference.version}`, imageUrl: reference.imageUrl!, source: 'Authority' as const })),
...
function AssetCard({ asset, usage, hasPoster, selected, onSelect, onDirect }: { asset: AssetSummary; usage?: AssetUsageSummary; hasPoster: boolean; selected: boolean; onSelect: () => void; onDirect: () => void }) {
  ...
  return <article className={`asset-card ${selected ? 'selected' : ''}`} draggable={!asset.isArchived} onDragStart={event => { event.dataTransfer.setData('text/asset-id', asset.id); event.dataTransfer.effectAllowed = 'move' }}>
    ...
    <div className="asset-card-actions"><DirectorModeButton active={false} subject={asset.displayName} onClick={onDirect} /></div>
...
function ImageRevisionWorkspace({ asset, collections, shots, references, onBack, onOpenGeneration, onChanged, onEditAuthority, ...director }: AssetDirectorControls & {
  ...
  useAssetDirectorView(active, director, compare && !activeIsCurrent)
...
function AssetInspector({ asset, collections, placements, originJob, shots, onClose, onSaved, onPlacementsChanged, onOpenGeneration, onOpenSequence, ...director }: AssetDirectorControls & { asset: AssetSummary; collections: AssetCollectionSummary[]; placements: AssetPlacementSummary[]; originJob?: JobSummary; shots: ShotSummary[]; onClose: () => void; onSaved: (asset: AssetSummary) => void; onPlacementsChanged: () => void; onOpenGeneration: () => void; onOpenSequence: () => void }) {
  ...
  useAssetDirectorView(asset, director)
  ...
  return <aside className="asset-inspector" aria-label={`${asset.displayName} details`}>
    <header><span><small>{asset.kind} · {asset.source}</small><strong>{asset.displayName}</strong></span><DirectorModeButton active={director.directorMode} onClick={() => director.onDirectorMode(!director.directorMode)} /><button onClick={onClose} aria-label="Close asset details"><X /></button></header>
...
function CreateAssetLauncher({ references, onOpenMusic, onClose, onImport, onOpenGeneration }: { references: AssetGenerationReference[]; onOpenMusic?: () => void; onClose: () => void; onImport: () => void; onOpenGeneration: (draft: AssetGenerationDraft) => void; onJobQueued: (job: JobSummary) => void }) {
  ...
  return <Dialog className="edit-dialog create-asset-dialog" onClose={onClose} labelledBy="create-asset-title">
    ...
    <footer><button className="secondary" onClick={onClose}>Cancel</button>{mode === 'image' ? <button className="primary" disabled={!name.trim()} onClick={openImageLab}>Open still-image studio<ArrowRight size={16} /></button> : <button className="primary" onClick={onOpenMusic} disabled={!onOpenMusic} title={onOpenMusic ? undefined : 'Music is placed on the sequence timeline. Add a shot first.'}>{onOpenMusic ? 'Open Sequence for music' : 'Add a shot first'}<ArrowRight size={16} /></button>}</footer>
  </Dialog>
}
