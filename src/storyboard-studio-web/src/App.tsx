...
import { Aperture, ArrowLeft, ArrowRight, BadgeCheck, Blocks, Bot, Boxes, Check, ChevronDown, CircleAlert, CircleDot, CircleSlash, Clapperboard, Command, Film, GalleryHorizontalEnd, Globe2, Grid2X2, Library, LoaderCircle, LockKeyhole, MessageCircle, RefreshCw, Search, Settings2, Sparkles, WandSparkles, X } from 'lucide-react'
...
import SampleWelcome from './components/SampleWelcome'
...
import type { AgentActivityEntry, AssetGenerationDraft, CodexAssistResponse, DirectorAssetView, DirectorSceneView, DirectorViewQuery, IntegrationSummary, JobSummary, PairingStatusSummary, ReferenceSummary, ShotRevisionProposalSummary, ShotSummary, StudioSnapshot, Workspace } from './types'
...
export default function App() {
  ...
  const assetViewRef = useRef<DirectorAssetView | undefined>(undefined)
  const noteAssetView = useCallback((view: DirectorAssetView | undefined) => { assetViewRef.current = view }, [])
  ...
  const refresh = useCallback(async (quiet = false) => {
    try {
      const next = await studioApi.snapshot(); setStudio(current => sameSnapshot(current, next) ? current : next); setSelectedId(current => current || next.shots[0]?.id || ''); if (!quiet) setError(null)
  ...
  // the workspace exits it, so the shell never stays hidden on another surface.
  useEffect(() => { setDirectorMode(false) }, [workspace])
  ...
  useEffect(() => {
    ...
    const unregister = registerFramewrightWebMcp({
      ...
      // The Scene workspace publishes what it has open through a ref, which does
      // not re-render this shell, so the packet is assembled when a tool asks
      // rather than when the shell last rendered. Otherwise an agent would be
      // told about the selection before last.
      getDirectorView: () => {
        const scene = sceneViewRef.current
        if (workspaceRef.current === 'scene') return scene
        if (workspaceRef.current === 'assets') return assetViewRef.current
        return workspaceRef.current === 'shot' ? directorViewRef.current : undefined
      },
      ...
      // A blockout plan is reviewed in the same inspector, for the same reason.
      onSceneBlockout: () => setSceneBlockoutSignal(value => value + 1),
      ...
      // An external tool may read context immediately after this call returns.
      // Commit navigation first, so that read describes the visible workspace.
      onInspectShot: (shotId, continuity) => flushSync(() => { setSelectedId(shotId); setWorkspace(continuity ? 'review' : 'shot'); setAgentOpen(true) }),
  ...
  const ratify = async () => {
    ...
    catch (e) { setError(e instanceof Error ? e.message : 'Could not approve this version.') }
  ...
  return <SampleArtContext.Provider value={Boolean(studio.project.isSample)}><div className={`app-shell${directorMode && (active === 'shot' || active === 'scene' || (active === 'assets' && !assetGenerationDraft)) ? ' director-mode' : ''}`}>
    ...
    <aside className="workspace-rail" aria-label="Workspaces">
      ...
      <div className="rail-bottom"><button onClick={() => { setSetupOpen(true); void inspectIntegrations() }}><Blocks /><span>Connect</span></button><button onClick={() => { setSetupOpen(true); void inspectIntegrations() }}><Settings2 /><span>Settings</span></button></div>
    </aside>
    <section className={`app-main workspace-${active}`}>
      <header className="context-bar">
        <div className="project-lead"><button className="project-back" onClick={() => setWorkspace('board')} aria-label="Return to shot board"><ArrowLeft size={17} /></button><ProjectSwitcher project={studio.project} creating={creatingProject} onCreatingChange={setCreatingProject} onError={setError} onSwitched={message => {
      ...
      <div id="studio-workspace" className="workspace-host" tabIndex={-1}>
        <ErrorBoundary key={active} onLeave={active === 'board' ? undefined : () => setWorkspace('board')}>
        {active === 'board' && <BoardWorkspace studio={studio} selectedId={selected?.id ?? ''} initialView={boardView} banner={studio.project.isSample ? <SampleWelcome key={studio.project.id} projectId={studio.project.id} onStartProject={() => setCreatingProject(true)} /> : undefined} onViewChange={setBoardView} onSelect={setSelectedId} onOpen={id => { setSelectedId(id); setWorkspace('shot') }} onCreateShot={initialImage => setEntityEditor({ kind: 'shot', initialImage })} onCreateAuthority={() => setEntityEditor({ kind: 'authority' })} onEditAuthority={authority => { setBoardView('authorities'); setSelectedAuthorityId(authority.id); setWorkspace('authority') }} onOpenLibrary={() => setLibraryOpen(true)} onChanged={message => { if (message) setToast(message); void refresh() }} />}
        {active === 'assets' && !assetGenerationDraft && <AssetWorkspace key={studio.project.id} directorMode={directorMode} onDirectorMode={setDirectorMode} onDirectorView={noteAssetView} studio={studio} initialAssetId={assetLandingId} onEditAuthority={authority => { setSelectedAuthorityId(authority.id); setWorkspace('authority') }} onOpenGeneration={draft => { setDirectorMode(false); setAssetGenerationDraft(draft) }} onOpenSequence={() => setWorkspace('sequence')} onJobQueued={acceptQueuedJob} onToast={setToast} />}
    ...
    {entityEditor?.kind === 'shot' && <ShotEditorDialog shot={entityEditor.shot} initialImage={entityEditor.initialImage} references={studio.references} suggestedCode={nextShotCode(studio.shots)} framesPerSecond={studio.project.framesPerSecond} onClose={() => setEntityEditor(null)} onSaved={saved => { setEntityEditor(null); setSelectedId(saved.id); setWorkspace('shot'); setToast(`${saved.code} ${saved.currentAssetId ? 'image draft created' : 'intent saved'}.`); void refresh() }} />}
    {entityEditor?.kind === 'authority' && <AuthorityEditorDialog onClose={() => setEntityEditor(null)} onSaved={saved => { setEntityEditor(null); setBoardView('authorities'); setSelectedAuthorityId(saved.id); setWorkspace('authority'); setToast(`${saved.name} v${saved.version} created as reference.`); void refresh() }} />}
  ...
  </div></SampleArtContext.Provider>
}

// While a job runs the shell polls every 700 ms. Most polls return exactly what
// is on screen; keeping the same object then means nothing re-renders.
function sameSnapshot(current: StudioSnapshot | null, next: StudioSnapshot) {
...
function JobsDock({ jobs, scopeShotId, onOpen, onRetry, dismissed, onDismiss }: { jobs: StudioSnapshot['jobs']; scopeShotId?: string | null; onOpen: () => void; onRetry: (job: JobSummary) => Promise<void>; dismissed: string[]; onDismiss: (id: string) => void }) {
  ...
  return <div className={`jobs-dock stalled ${failed ? 'is-failed' : 'is-cancelled'}`} role="alert" data-testid="jobs-dock-stalled">
    ...
    <span>
      ...
      <small>{stalled.error || (failed ? 'The renderer did not return a version.' : 'This job was stopped before it finished.')}</small>
    </span>
