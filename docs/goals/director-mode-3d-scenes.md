...
- **Current MVP increment:** M12 has an ordinary UI path for replacing one selected placeholder with an exact library revision while preserving instance identity, transform, plan provenance, annotations, pivot/motion, camera, lighting, and unrelated objects. Scene Director Mode uses the same working state. Its read-only `search_scene_assets` tool now finds exact current, non-archived model revisions in the active project so the assistant can reuse the library before suggesting generation. M16 freezes a saved scene version, distinct shot camera, frame range, selected time, project delivery contract, and snapshot hash; the real Three.js canvas produces a delivery-sized PNG and advances the existing shot candidate/review path. M17 now starts only from that ratified still, freezes the scene/assets/timing/cameras/format/encoder, draws exact scene times through the same Three.js renderer, resumes validated PNG frames by index after restart, and uses local FFmpeg to create a probed Max take in existing shot review. Working-package schema v5 carries the completed job/manifest/video lineage and complete editable project graph. Its importer now remaps the operational M17 packet alongside relational IDs while retaining immutable source manifests and snapshot hashes; completed takes do not falsely request omitted scratch frames after import.
- **Validation:** The complete `scripts/verify.ps1` gate passed in one invocation on isolated port 5210: dependency audits, frontend checks/build, public-content audit, locked restore, warning-free Release rebuild, 289 backend tests, all backup/setup/worker/release scripts, voice and YuE2 contract tests, NuGet vulnerability inspection, and 136 Playwright journeys with six intentional tablet exclusions. The known Three.js chunk-size advisory remains; scene modules are lazy-loaded. The import proof exports, verifies, imports, activates, reopens, and serves the model from a fresh project, then imports the same package again with fresh IDs. A controlled completed M17 take also survives fresh-ID import, a new application host, playback, and re-export. Negative controls reject a checksum failure and a self-consistent forged GLB without creating a partial project.
...
- **Hosted CI:** Public `main` at `bfb57de2e66c28f0a0e365407d031d0cf8ead704` passed [GitHub Actions run 35709626479](https://github.com/raydeStar/framewright/actions/runs/35709626479): build/API, container runtime, and all browser acceptance checks were green. It includes the M17 baseline, portable recovery extension, reusable-asset discovery, Director-view synchronization, and initial asset-library interaction gate.
...
### Reusable asset discovery checkpoint - 2026-09-22

Status: The dependency-ready Director Mode convenience is contract-verified. This checkpoint does not advance a human- or live-worker-gated milestone.

`search_scene_assets` exposes a bounded, project-scoped read of current, non-archived model revisions, including exact asset and revision identity, content hash, tags, preparation verdict, and scene usage count. A returned `assetId` can be passed to the existing `matchAssetId` blockout contract. Search starts no import, placement, generation, provider call, or durable job. Literal SQL wildcards are escaped, overlong searches are refused, and project switching cannot leak another project's library.

The first complete gate exposed a tablet race after Save: the page already displayed the new saved version while the passive effect that publishes Director context still carried `dirty: true`. Publishing that imperative view in a layout effect now makes the browser-tool snapshot change in the same committed UI update. The formerly failing tablet journey passed five isolated repeats and then the complete matrix. A later slow hosted tablet run exposed a second interaction race while the asset library's initial snapshot committed between pointer-down and pointer-up on **Create new**. Import and creation now wait for that initial load, the workspace publishes its busy state, and a delayed-response regression passed ten tablet repeats. The final `scripts/verify.ps1` run passed 289 backend tests and 136 browser journeys with six intentional tablet exclusions. Codex's in-app browser then discovered the exact eleven closed-schema registrations from an isolated current build, including `search_scene_assets` with read-only annotations. Public commit `bfb57de2e66c28f0a0e365407d031d0cf8ead704` passed [hosted run 35709626479](https://github.com/raydeStar/framewright/actions/runs/35709626479) across build/API, container runtime, and browser acceptance with the loading gate in place. The existing Three.js bundle-size advisory remains. No provider, GPU worker, compiler checkout, production data, or external queue was touched.
...
### Portable recovery groundwork - 2026-09-21, extended 2026-09-22
...
Evidence: export -> import -> activate -> reopen -> model-content retrieval passes; a second import receives
  ...
  partial project. Desktop and tablet project-switcher journeys pass in the complete gate. On the self-contained
  Windows package, a disposable image and editable project round-trip through export/import; a backup is taken,
  a distinct post-snapshot asset is added, the packaged process stops, the packaged offline restore tool replaces
  the snapshot exactly, and a new packaged process reopens the imported project with matching asset bytes/hash,
  no post-snapshot asset, and SQLite quick-check `ok`. A 2026-09-22 extension completes a controlled M17 take,
  ratifies its video, exports it, imports it with fresh operational and relational IDs, starts a new application
  host, serves the playable movie, and re-exports the imported graph. Immutable source manifests/snapshot hashes
  remain exact while the operational packet addresses the new scene, shot, binding, job, and asset identities.
  The artist-facing app remains live on a different root/port.
Limit: this is not M18 completion. The full small-workshop graph with the accepted M14 rig and real M17 take on the
  self-contained packaged runtime, its complete packaged restore, and human/physical-device evidence remain open.
```
