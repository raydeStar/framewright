...
- **Validation:** The complete `scripts/verify.ps1` gate passed in one invocation on isolated port 5204: dependency audits, frontend checks/build, public-content audit, locked restore, warning-free Release rebuild, 288 backend tests, all backup/setup/worker/release scripts, voice and YuE2 contract tests, NuGet vulnerability inspection, and 134 Playwright journeys with six intentional tablet exclusions. The known Three.js chunk-size advisory remains; scene modules are lazy-loaded. The import proof exports, verifies, imports, activates, reopens, and serves the model from a fresh project, then imports the same package again with fresh IDs. A controlled completed M17 take also survives fresh-ID import, a new application host, playback, and re-export. Negative controls reject a checksum failure and a self-consistent forged GLB without creating a partial project.
...
- **Hosted CI:** M17 baseline `bee78f8` passed [GitHub Actions run 35695942853](https://github.com/raydeStar/framewright/actions/runs/35695942853): build/API, container runtime, and all browser acceptance checks were green. The portable M17 recovery extension described below is newer and must receive its own exact-head hosted result after push.
