...
The first complete gate exposed a tablet race after Save: the page already displayed the new saved version while the passive effect that publishes Director context still carried `dirty: true`. Publishing that imperative view in a layout effect now makes the browser-tool snapshot change in the same committed UI update. The formerly failing tablet journey passed five isolated repeats and then the complete matrix. The final `scripts/verify.ps1` run on port 5210 passed 289 backend tests and 136 browser journeys with six intentional tablet exclusions. Codex's in-app browser then discovered the exact eleven closed-schema registrations from an isolated current build, including `search_scene_assets` with read-only annotations. Public commit `f5b00155df3aa659df478f5409be231a3d883869` passed [hosted run 35705366882](https://github.com/raydeStar/framewright/actions/runs/35705366882) across build/API, container runtime, and browser acceptance. The existing Three.js bundle-size advisory remains. No provider, GPU worker, compiler checkout, production data, or external queue was touched.
