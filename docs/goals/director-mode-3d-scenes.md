...
- **Next action:** M08 is in progress and its groundwork is done. The compiler now offers a `geometry` stage: one reference image in, one candidate mesh out, with the workspace, intake and request written for it, and a capability answer that names which of `legacy-root`, `geometry-environment`, `hunyuan-checkout` and `runners` is missing on a machine without the weights. This studio's gateway can name that studio tree (`Integrations:ReferenceAssetCompiler:StudioTreePath`) so the compiler is told where the weights are rather than left to find them. No inference has been run.
  Three things remain. First, the route: M07 freezes a single stage into its request and runs it on the reference image, but `browser-payload` takes a mesh, not a picture. The real route is two stages -- `geometry` on the image, then `browser-payload` on the candidate -- so the frozen request must carry a route rather than one stage name, and reconciliation must account for two intermediate artifacts. That work needs no GPU and can be proven with a controlled compiler, exactly as M07 was. Second, this workstation must be configured: `BlenderPath` and `StudioTreePath` are both empty, so the live preflight currently answers "blender missing". Third, the artist must authorize the run and record visual acceptance, which the milestone requires and which nothing here can supply.
